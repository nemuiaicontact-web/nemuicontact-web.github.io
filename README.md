...
画面下部に、HOME / ABOUT / WORKS / LABへすぐ移動できるナビゲーションを表示します。

WORKSは最初の3件だけを表示し、続きがある場合は
`MORE WORKS — 残り○件` ボタンから残りを表示できます。
タグを切り替えると、そのタグの最初の3件へ戻ります。

作品タグは横一列でスクロールできるため、タグが増えても縦幅を取りすぎません。
横へ動かせることが分かるように、黄色の `SWIPE TO FILTER →` と
右端のフェード・矢印を表示します。タグ一覧の右端まで移動すると、右端の案内は消えます。
これらの省スペース表示は、画面幅560px以下で自動的に有効になります。
...
- スマホ用のHOME / ABOUT / WORKS / LAB下部ナビゲーションを追加
- スマホではWORKSを最初の3件にまとめ、残りをボタンで表示する方式へ変更
- スマホの作品タグを横スクロール式に変更
- スマホのセクション余白を調整し、縦スクロール量を軽減
- WORKSの説明文で改行を表示できるように調整
- スマホのタグ一覧へ黄色の横スワイプ案内と右端の続きサインを追加
- LABの下へ著作権表記と運営開始日を記載したフッターを追加
